...
- **全域 AI 调度架构升维 (Next Action)**：
  - 确立 [全域 AI 异步任务调度中心 4.0] 方案，解决跨页面生成与物理熔断 (`AbortController`) 的序列化灾难。
- 修复：斩断 DailyPlanGrid 与 useStudyPlan 的状态撕裂，统一消费 useUserStore 的持久化数据，根除无限 Loading 死锁。
- 重构：完成高危 UI 物理收口，将 [重新评估] 按钮微拟态化并移至侧边栏 ProfilePanel。
- 强化：在 PretestBoard 组装 payload 时注入系统级 Negative Prompt，彻底压制 LLM 复习“已掌握知识点”的幻觉。
- 备忘：排查出 Token 物理死亡导致的 401 僵尸态死锁问题，已将“全局 401 物理斩断与弹射机制”记录至技术债台账。

### 全域 AI 异步任务调度中心 4.0 (Dual-Track State Machine)
- 纯内存调度池 (activeAITasks)：存储不可序列化的 AbortController，掌控跨页长连接的生杀大权，F5 刷新即毁。
- 持久化战利品池 (aiAssets)：唯一且永久的 AI 产物真理源 (C2V/K2V/3D)，结构化存储 AIAssetInfo，受 persist 保护。
- 解耦红线：UI 视图层剥离 AbortController 实例化逻辑，仅消费大脑状态（渲染 Loading 或 停止按钮）；推荐池 (Recommendation Pool) 作为静态规则硬编码，严禁混入动态资产池。
...
## 拾、 全域 AI 异步任务调度与状态机架构 (Global AI Dispatch & State Architecture)

### 1. 单一状态真理源 (SSOT) 与沉浸页拓扑
所有业务状态（用户画像、测试结果、学习计划、已掌握知识点）必须 100% 驻留在 `useUserStore`。UI 组件仅作为状态的映射器 (Mapper) 与动作的分发器 (Dispatcher)。
沉浸页 (Knowledge Node) 必须严格遵守以下物理拓扑：
- 左侧 80% 主脑区：`NodeK2VBlock` (浅色微拟态视频降维) + `Node3DBlock` (具身交互沙盒)。
- 右侧 20% 陪伴区：`NodeRightSidebar` 挂载独立状态罗盘与对话流媒体插槽。

### 2. 双轨分离状态机 (Dual-Track State Machine 4.0)
为实现全站 AI 任务（K2V/C2V/3D）的“后台守护进程”与“跨页面随时熔断”，状态机必须严格执行内存与硬盘的物理隔离：
- **动态运行池 (Volatile Task Map - 内存绝对隔离)**：
  - 状态节点：`activeAITasks: Record<string, AbortController>`
  - 🚨 致命红线：`AbortController` 属于浏览器原生内存指针，**严禁**被 Zustand 的 `persist` 中间件收集并写入 `localStorage`！否则将导致反序列化崩溃与应用无限期白屏。必须在 Zustand 中通过白名单/黑名单机制 (`partialize`) 实施物理隔离。
  - 生命周期：组件挂载发起生成时写入内存池；触发物理熔断或生成结束时，立刻从字典中抹除，释放内存。
- **战利品仓库 (Persisted Result Map - 硬盘烙印)**：
  - 状态节点：`aiAssets: Record<string, string>` (存储物理视频 Blob URL 或 3D 资产图纸)。
  - 生命周期：AI 降维成功后写入，受 `persist` 保护。页面跳转/刷新后直接从此处水合资产，实现无等待的瞬间渲染。

### 3. 全局熔断与水合契约 (Global Circuit Breaker Contract)
各 AI 业务线（含工具台与详情页）在处于 `running` 状态时，必须在 UI 层渲染 `[ 🛑 停止生成 ]` 显式熔断按钮。该动作只允许调用 Store 暴露的全局 `abortAITask(taskId)` 方法，精准触发底层 TCP/SSE 物理断连。**严禁**在组件内使用局部的 `useRef<AbortController>` 制造状态孤岛与幽灵任务。
